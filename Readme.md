...
8*8 LED点阵由8行8列共64的LED点组成，LED点可以是圆形的也可以是方形的，我们这里使用的是圆形的。

![image-20251017135554969](./assets/image-20251017135554969.png)

![image-20251017140032551](./assets/image-20251017140032551.png)
...
- 像素直径
  ...
  像素直径即一个LED点的直径，一般有1.9mm、3mm、3.5mm、5mm等不同规格

  ![image-20251017140454973](./assets/image-20251017140454973.png)

- 点间距

  两个像素点圆心之间的距离，数值越小显示越细腻
...
- 引脚分布
  ...
  ![image-20251017140749840](./assets/image-20251017140749840.png)

  以1.9mm的共阴8*8 LED点阵788AS为例，内部电路原理图和引脚分布图如上。

  注意：并不是上面一排管脚控制行，下面一排管脚控制列；也不是上面一排管脚控制列，下面一排管脚控制行。

  第9、14、8、12、1、7、2、5号管脚分别控制第1、2、3、4、5、6、7、8行。

  第13、3、4、10、6、11、15、16号管脚分别控制第1、2、3、4、5、6、7、8列。

  如想要第一行第一列的LED亮，则13脚给高电平，9脚给低电平，这样就点亮第一行第一列的LED。

##### MAX7219

**概述**

MAX7219 是一款常用的SPI串行接口 LED 显示驱动芯片，专门用于驱动 8 位共阴极数码管或64 只独立 LED（LED点阵），广泛应用于各类嵌入式显示场景，其主要特性如下：

- SPI串行通信：通信速率高达10MHz，仅需要3根控制线（DIN、CLK、CS）即可以与单片机进行通信，节省IO口
- 驱动能力：单芯片可驱动8个数码管（共阴极）或8x8 LED点阵
- 内置功能：集成8*8静态RAM、集成段码译码器、亮度可调节、支持自动动态扫描且扫描界限可设置、数码管消隐和测试模式
- 支持级联：可将多个MAX7219进行级联，第一个芯片的DOUT引脚接第二个芯片的DIN引脚，所有芯片的CLK、CS引脚并联

**引脚分布和功能**

![image-20260309154636877](./assets/image-20260309154636877.png)

- DIN，SPI数据输入引脚
- DIG0~DIG7，共阴极控制引脚，使能时为低电平
- CS，SPI选择引脚
- CLK，SPI时钟引脚
- SEGA~G\DP，显示数据控制引脚，使能时为高电平
- ISET，峰值电流设定，一般选择固定10K欧的电阻与电源相连
- DOUT，SPI数据输出引脚，用于芯片级联

**数据格式和时序**

![image-20260309160327615](./assets/image-20260309160327615.png)

每次传输采用16位数据传输，高位MSB在前，第一个字节为寄存器地址，参见寄存器地址分布，第二个字节为数据
...
![image-20260309160644922](./assets/image-20260309160644922.png)



**寄存器地址分布**

![image-20260309155843634](./assets/image-20260309155843634.png)

- No-Op寄存器

  在级联时，No-Op寄存器可以发挥作用。假如有四个MAX7219级联，需要向第4个MAX7219，前三个MAX7219的数据不变：先写入第4个MAX7219需要的16bits数据，然后发送3个No-Op数据，可以实现期望效果

- 译码控制寄存器

  ![image-20260309165326480](./assets/image-20260309165326480.png)

  MAX7219有两种译码方式：B译码方式和不译码方式：

  - 当选择不译码时，8个bit数据分别对应7个段和小数点位（SEGF~A和DG）
  - B译码方式是BCD译码，直接送BCD数据就可以显示，内部会自动转换成7段式共阴数码管的显示格式

- 亮度控制寄存器

  ![image-20260309163719765](./assets/image-20260309163719765.png)

  通过调节段电流与ISET峰值电流的比例，调节亮度

- 扫描界限寄存器

  ![image-20260309162818165](./assets/image-20260309162818165.png)

  设置需要显示的digit数量，一般为7，如果实际硬件不足7个，则根据情况设置

- 关断模式寄存器

  ![image-20260309164827792](./assets/image-20260309164827792.png)

  关断模式（D0=0），正常模式（D0=1）

- 显示测试寄存器

  ![image-20260309164306039](./assets/image-20260309164306039.png)

  正常工作状态(D0=0)，测试状态（D0=1）。当测试状态时，各位显示全亮
